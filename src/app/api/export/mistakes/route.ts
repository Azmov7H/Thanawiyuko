...
import { generateMistakeReportPdf } from "@/server/modules/exports";
...
/** GET /api/export/mistakes — Plus-gated mistake-library report PDF (T-K2). */
export async function GET() {
  ...
  try {
    const pdf = await generateMistakeReportPdf(userId);
    ...
    await logServerError("export.mistakes.failed", e, { route: "/api/export/mistakes" });
